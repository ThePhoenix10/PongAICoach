...
import { ApiError, createUpload, deleteJob, getLimits, uploadToBlob } from "@/lib/api";
...
export function UploadPanel({
  ...
  // Track the job id so we can delete it if the user cancels mid-upload
  ...
  // Cancel the in-progress upload AND delete the job record from the backend
  // so it doesn't appear in the "Videos uploaded" list as a ghost entry.
  const cancel = async () => {
    ...
    if (id) {
      try {
        ...
        // Silently ignore — job will show as "Not submitted" and user can
        // manually delete it from the list.
      }
      jobIdRef.current = null;
    }
    reset();
  ...
  async function startUpload() {
    ...
    try {
      ...
      // Store job id so cancel() can delete it if needed
      ...
      if (e instanceof DOMException && e.name === "AbortError") {
        // cancel() already handled cleanup
        return;
      }
